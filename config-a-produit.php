        ...
        <div class="container">
            ...
            <div class="row justify-content-center features">
                <div class="col-sm-6 col-md-5 col-lg-4 item">
                    ...
                    $submit = $_POST['submit'];
                    if ($submit == "Connexion") {
                        $sqlQuery = "SELECT * from produit WHERE nom LIKE '$identifiant%'";
                        $verifStatement = $mysqlClient->prepare($sqlQuery);
                        $verifStatement->execute();
                        $verif = $verifStatement->fetchAll();
                        foreach ($verif as $verif) {
                            $sqlQuery = 'SELECT libelle FROM categorie WHERE categorie_id = ' . $verif["categorie"];
                            ...
                            $nom = $verif['nom'];
                            $comp = 0;
                            $container = '<div class="box"><div class="box"><img height="100px"src="images/' . $stock[0]["libelle"] . '/' . $image . '" alt="error"></i>
                                            <h3 class="name">' . $nom . '</h3>
                                            <p class="description">Stock Actuel : ' . $lestocks . '</p>
                                            <form methode="post">
                                                <input placeholder="nouveau stock" name="stock' . $id . '" type="number">
                                                <div class="col" style="padding-top:40px;">
                                                    <button class="btn btn-dark" name="' . $nom . '" type="submit" style="background-color:#00b760;">M O D I F I E R</button>
                                                </div>
                                            </form>   
                                        ';
                            echo $container;
